...
| Hiring | [`contractor-or-employment-structuring`](skills/hiring/contractor-or-employment-structuring/) | Organize how a contractor engagement actually operates against recognized factors, preserve each factor's evidence, size retroactive cost from user-supplied assumptions, and test which employment-like factors a proposed change removes. |
...
| Sales | [`customer-contract-terms-review`](skills/sales/customer-contract-terms-review/) | Convert billing schedule, acceptance lag, and payment terms into a cash path with the peak self-funded amount, measure liability, termination, and renewal exposure against user-defined limits, and order negotiation priorities. |
...
| Operations | [`security-questionnaire-readiness`](skills/operations/security-questionnaire-readiness/) | Inventory a customer security check sheet, count only evidence-backed items as answerable, list the must-level gaps that block the deal, and back-schedule remediation against the submission deadline. |
...
| Management | [`annual-operating-plan`](skills/management/annual-operating-plan/) | Turn monthly revenue, margin, cost, and committed-outflow assumptions into a twelve-month cash path, test whether that arithmetic reaches annual targets, and cascade the result into quarterly checkpoints with revision triggers. |
