...
description: Calibrate a sales forecast using historical error, bias, stage conversion ranges, sample quality, and current pipeline coverage. Use when a founder needs a defensible revenue range and required pipeline gap; do not use as a revenue guarantee or permission to alter CRM records.
license: MIT
metadata:
  author: ficilcom
---
...
3. 各入力に `confirmed`、`reported`、`estimated`、`unknown` を付ける。標本が小さい率、創業者判断、期限超過案件を明示する。
...
5. [報告書形式](references/report-format.md)で予測範囲、誤差、偏り、目標差、案件不足、確認が必要な案件と次回校正日を示す。
